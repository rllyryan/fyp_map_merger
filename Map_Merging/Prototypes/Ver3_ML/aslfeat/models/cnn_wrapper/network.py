...
class Network(object):
    ...
    def __init__(self, inputs, is_training,
        ...
        self.regularizer = slim.l2_regularizer(1.0) if regularize else None
    ...
    def deform_conv(self,
        ...
        def _get_conv_indices(feature_map_size, kernel_size, strides, dilation_rate):
            ...
            x, y = [tf.image.extract_patches(i,
                                             ...
                                             'VALID') for i in [x, y]]  # shape [1, out_h, out_w, filter_h * filter_w]
            return y, x
        ...
        if self.training:
            in_h, in_w = [inputs.get_shape()[i] for i in range(1, 3)]
            ori_h, ori_w = [input_tensor.get_shape()[i] for i in range(1, 3)]
        else:
        ...
        with tf.compat.v1.variable_scope('deform_param', reuse=reuse):
            if deform_type == 'a':
                ...
                if False:
                    ...
                    a11_new = det / b2a2
                    A = tf.stack([tf.stack([a00_new, a01_new], axis=-1),
                                  tf.stack([a10_new, a11_new], axis=-1)], axis=-2)
                ...
                if self.training:
                    offset = tf.reshape(offset, [batch_size, comb_A.get_shape()[1],
                ...
                offset = tf.stack([offset[:, :, :, :, 1], offset[:, :, :, :, 0]], axis=-1)
                if self.training:
                    offset = tf.reshape(offset, [batch_size, H_mat.get_shape()[1],
        ...
        if self.training:
            out_h, out_w = [offset.get_shape()[i] for i in range(1, 3)]
        else:
        ...
        if False:
            ...
            with tf.compat.v1.variable_scope(name, reuse=self.reuse):
                ...
                out = tf.nn.conv3d(input=pixels, filters=weights, strides=[
                    1, 1, 1, 1, 1], padding='VALID', name=None)
                out = tf.squeeze(out, axis=-2)

            if biased:
                bias = tf.compat.v1.get_variable(
                    'bias', (filters, ), dtype=tf.float32, initializer=bias_init)
    ...
    def deform_conv_bn(self,
                       ...
                       padding=DEFAULT_PADDING,
                       reuse=False):
        deform_conv = self.deform_conv(input_tensor, kernel_size, filters, strides, name, relu=False,
                                       deform_type=deform_type, modulated=modulated,
                                       dilation_rate=dilation_rate, padding=padding,
                                       biased=biased, reuse=reuse)
        deform_conv_bn = self.batch_normalization(deform_conv, name + '/bn',
                                                  center=center, scale=scale, relu=relu, reuse=reuse)
